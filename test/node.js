...
test('access null links', async (t) => {
  ...
  async function access (getLink) {
    let ntx = simDb()
    t.is(await getLink.call(node, ntx), null)
    t.is(ntx.gets.length, 0)
  }

  await access(node.left)
  await access(node.right)
  await access(node.parent)
  await access((ntx) => node.child(ntx, true))
  await access((ntx) => node.child(ntx, false))
})
...
test('save child node', async (t) => {
  ...
  t.is(ntx.gets.length, 0)
  t.is(ntx.dels.length, 0)
  t.deepEqual(ntx.puts, [
    {
      key: 'nfoo',
  ...
  let ntx2 = simDb()
  t.is(await node.parent(ntx2), null)

  // get parent
  ntx = simDb(ntx)
  let parent = await node2.parent(ntx)
  t.is(parent.key, 'foo')
  t.is(parent.value, 'bar')
  t.deepEqual(ntx.gets, [ { key: 'nfoo' } ])

  // get child
  ntx = simDb(ntx)
  let child = await node.left(ntx)
  t.is(child.key, 'fo')
  t.is(child.value, 'bar')
  t.deepEqual(ntx.gets, [ { key: 'nfo' } ])
})
...
test('delete child node', async (t) => {
  ...
  ntx = simDb(ntx)
  node = await node.delete('fo', ntx)

  t.deepEqual(ntx.gets, [ { key: 'nfo' } ])
  t.deepEqual(ntx.dels, [ { key: 'nfo' } ])
  t.deepEqual(ntx.puts, [
    {
      key: 'nfoo',
      value: 'gdGeF1+rlOY3B4TKbNE1jNesAMmtBZ8KwnM2IWh629jJxBTapVD+iAAAA2JhcgAAAA=='
...
test('delete parent node', async (t) => {
  ...
  ntx = simDb(ntx)
  node = await node.delete('foo', ntx)

  t.deepEqual(ntx.gets, [ { key: 'nfo' } ])
  t.deepEqual(ntx.dels, [ { key: 'nfoo' } ])
  t.is(ntx.puts.length, 0)
  t.is(node.key, 'fo')

  ntx = simDb(ntx)
  t.is(await node.parent(ntx), null)
})
